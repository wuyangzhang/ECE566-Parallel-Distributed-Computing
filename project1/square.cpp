...
int main(int argc, char *argv[]){
	...
	for(unsigned int i = 0 ; i < 3; i ++){
		reset_and_start_timer();
		calRoot_ispc_task_exe(totalNum, values, result, firstGuess);
		double dt = get_elapsed_mcycles();
    	printf("@time of ispc task run:\t\t\t[%.3f] million cycles\n", dt);
        minISPC_task = std::min(minISPC_task, dt); 
	}
    printf("[root calculate ispc task ]:\t\t[%.3f] million cycles\n", minISPC_task);
    ...
    printf("\t\t\t\t(%.2fx speedup from ISPC_task)\n", minSerial/minISPC_task);
